...
conf_int = np.percentile(np.array(bs_replicates),[2.5,97.5])
...
p = np.sum(perm_replicates <= np.mean(nht_diff_obs)) / len(perm_replicates)
print('p-val =', p)
...
conf_int = np.percentile(np.array(bs_diff_replicates),[2.5,97.5])
